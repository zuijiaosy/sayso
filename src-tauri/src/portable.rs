...
/// Magic string written into the `portable` marker file.
const PORTABLE_MARKER: &str = "Sayso Portable Mode";

/// Markers accepted when detecting an existing portable install. The upstream
/// string stays valid forever: an installed portable copy keeps its marker on
/// disk across updates, and rejecting it would strand that install's `Data/`
/// dir. `nsis/installer.nsi` accepts the same two.
const ACCEPTED_PORTABLE_MARKERS: [&str; 2] = [PORTABLE_MARKER, "Handy Portable Mode"];
...
/// Check if a marker file path contains a recognized portable magic string.
/// Extracted for testability.
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| {
            let s = s.trim();
            ACCEPTED_PORTABLE_MARKERS
                .iter()
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        let dir = std::env::temp_dir().join("sayso_test_valid");
    ...
    #[test]
    fn test_legacy_upstream_magic_string_still_enables_portable() {
        // A portable install created before the rename keeps its old marker on
        // disk; rejecting it would strand that install's Data/ dir.
        let dir = std::env::temp_dir().join("sayso_test_legacy_magic");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
    ...
    fn test_empty_file_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("sayso_test_empty");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
    ...
    fn test_wrong_content_does_not_enable_portable() {
        let dir = std::env::temp_dir().join("sayso_test_wrong");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
    ...
    fn test_legacy_empty_marker_without_data_dir_does_not_enable_portable() {
        ...
        let dir = std::env::temp_dir().join("sayso_test_legacy_no_data");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        std::fs::File::create(&marker).unwrap();
        assert!(!is_valid_portable_marker(&marker));
    ...
    fn test_magic_string_with_whitespace_enables_portable() {
        let dir = std::env::temp_dir().join("sayso_test_ws");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        writeln!(f, "  Sayso Portable Mode").unwrap();
        assert!(is_valid_portable_marker(&marker));
        std::fs::remove_dir_all(dir).unwrap();
    }
